...
#' \code{F}: time series of core tensor, stored in a 'Tensor' object, where mode-1 is the time mode \cr
#' \code{E}: time series of error tensor, stored in a 'Tensor' object, where mode-1 is the time mode \cr
#' @export
  ...
  # output$E: time series of error tensor
{
  ...
  f_series = as.tensor(array(numeric(),c(T,r)))
  unfold_f = unfold(f_series,row_idx = 1, col_idx = 2:(K+1))
  for (j in 1:prod(r))
  {
    unfold_f[,j] = arima.sim(n = T, n.start = 6, list(ar = c(0.7,0.3,-0.4,0.2,-0.1)))
  }
  f_series = fold(unfold_f,row_idx = 1, col_idx = 2:(K+1),modes = c(T,r))
  ...
  common_e = array(numeric(),c(T,re))          # cross-sectional common error
  idio_e = as.tensor(array(numeric(),c(T,d)))
  ...
  for (i in 1:re)
  {
    common_e[,i] = arima.sim(n = T, n.start = 6, list(ar = c(-0.7,-0.3,-0.4,0.2,0.1)))
  }
  ...
  for (j in 1:prod(d))
  {
    unfold_idio_e[,j] = arima.sim(n = T, n.start = 6, list(ar = c(0.8,0.4, -0.4,0.2,-0.1)))
  }
  idio_e = fold(unfold_idio_e,row_idx = 1, col_idx = 2:(K+1),modes = c(T,d))
  ...
  e_series = as.tensor(array(numeric(),c(T,d)))
  ...
  e_series = fold(unfold_e,row_idx = c(main_d + 1, 1), col_idx = c(1:(K+1))[-c(1, main_d + 1)], modes = c(T,d))


  # generate the tensor time series X
  X = as.tensor(array(numeric(),c(T,d)))
